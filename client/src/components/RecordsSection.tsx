...
import { DownloadButton } from './DateRange'
...
/**
 * Self-contained records section for a single record type.
 *   type="buyer"    → revenue records (Call Records page)
 *   type="campaign" → cost records   (Campaigns page, below the grid)
 *
 * `compact` renders a section-level header (h2 + divider) instead of a page header.
 * `onChange` fires after a create/update/delete so a parent can refresh its own data.
 */
  ...
  const actions = (
    <>
      ...
      <Button variant="secondary" onClick={handlePdf} disabled={pdfLoading}>
        {pdfLoading ? <Spinner className="h-3.5 w-3.5" /> : <PdfIcon />}
        PDF
      </Button>
...
function RecordForm({
  ...
  const [entityId, setEntityId] = useState<string>(
    editing ? String((isBuyer ? editing.buyer_id : editing.campaign_id) ?? '') : '',
  )
  ...
  const handleAnswered = (v: string) => {
    setAnswered(v)
    setCounted(String((Number(v) || 0) + (Number(missed) || 0)))
  }
  const handleMissed = (v: string) => {
    setMissed(v)
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        {creatingNew && (
          <Input label="New code" placeholder={isBuyer ? 'e.g. RTG 99' : 'e.g. C-12'} value={newCode} onChange={(e) => setNewCode(e.target.value)} required />
        )}
      </div>
