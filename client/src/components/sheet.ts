...
/**
 * A date cell. A native date field is wider than it looks — "mm/dd/yyyy" plus the calendar
 * button — and in a narrow column the browser clips the year rather than shrinking it. So
 * the padding is pulled in, the digits go a size down, and the calendar button is shrunk,
 * which is enough for the whole date to show in a ~10% column of a wide sheet.
 */
export const dateFieldCls = cx(
  fieldCls,
  'px-1 text-[11px] tabular-nums',
  '[&::-webkit-calendar-picker-indicator]:m-0 [&::-webkit-calendar-picker-indicator]:h-3',
  '[&::-webkit-calendar-picker-indicator]:w-3 [&::-webkit-calendar-picker-indicator]:p-0',
