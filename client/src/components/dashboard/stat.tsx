...
export default function DashboardStat({
  ...
  // Extract prefix, numeric value, and suffix. Commas are stripped so currency
  // like "$12,345" parses; NumberFlow re-applies locale grouping on render.
          ...
          <div className="absolute top-0 right-0 w-14 h-full pointer-events-none overflow-hidden group">
            ...
            >
              {[0, 1].map((block) => (
                <div
                  key={block}
                  className={cn("flex", direction === "up" ? "flex-col-reverse" : "flex-col")}
                >
...
const Arrow = ({ direction, index }: { direction: "up" | "down"; index: number }) => {
  ...
  return (
    <span
      ...
      className={cn(
        ...
        "transition-all duration-700 ease-out animate-marquee-pulse will-change-transform",
      )}
    >
      {direction === "up" ? "↑" : "↓"}
    </span>
