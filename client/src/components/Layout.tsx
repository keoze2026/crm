  ...
  return (
    <div className="flex min-h-screen text-slate-900">
      ...
      <div className="flex-1 lg:pl-60">
        {/* Mobile top nav */}
        <div className="glass-strong sticky top-0 z-20 flex items-center gap-1 overflow-x-auto border-b border-white/50 px-3 py-2 lg:hidden">
          {NAV.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
              ...
              className={({ isActive }) =>
                cx(
                  ...
                  isActive ? 'bg-linear-to-r from-blue-500 to-blue-600 text-white shadow' : 'text-slate-600',
                )
              }
            >
              {item.label}
