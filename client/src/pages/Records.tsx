  ...
  return (
    <div>
      <PageHeader title="Call Records" subtitle="Daily call volumes entered by the team">
        <DownloadButton href={api.recordsExportUrl(filters)}>Export CSV</DownloadButton>
...
function nextDir(filters: RecordFilters, col: string): 'asc' | 'desc' {
  if (filters.sort === col) return filters.dir === 'asc' ? 'desc' : 'asc'
  return 'desc'
}
...
function Th({ children, onClick, sortable, active, dir }: { children: React.ReactNode; onClick?: () => void; sortable?: boolean; active?: boolean; dir?: string }) {
  return (
...
function ThNum({ children, onClick, active, dir }: { children: React.ReactNode; onClick: () => void; active: boolean; dir?: string }) {
  return (
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        {isEdit && (
          <Input label={editing!.record_type === 'buyer' ? 'Buyer' : 'Campaign'} value={editing!.buyer_code ?? editing!.campaign_code ?? ''} disabled />
        )}
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Input label="Answered" type="number" min="0" value={answered} onChange={(e) => setAnswered(e.target.value)} />
        <Input label="Missed" type="number" min="0" value={missed} onChange={(e) => setMissed(e.target.value)} />
        <Input label="Counted" type="number" min="0" value={counted} onChange={(e) => setCounted(e.target.value)} />
