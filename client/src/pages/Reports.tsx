...
function buildCompleteReportPdf(data: CompleteReport): jsPDF {
  ...
  if (buyerBody.length === 0) {
    buyerBody.push([dateLabel, '—', '—', '—', '—', '—', '—'])
  }
  // Append totals as last body row (not foot — foot repeats on every page)
  const buyerTotalRow: RowInput = [
    'TOTAL', String(bt.destinations), num(bt.answered), num(bt.missed),
    num(bt.counted), '—', money2(bt.total_bill),
  ]
  buyerBody.push(buyerTotalRow)
  const buyerTotalIndex = buyerBody.length - 1
  ...
  autoTable(doc, {
    ...
    didParseCell: (data) => {
      if (data.section === 'body' && data.row.index === buyerTotalIndex) {
  ...
  if (campBody.length === 0) {
    campBody.push([dateLabel, '—', '—', '—', '—', '—', '—', '—'])
  }

  const campTotalRow: RowInput = [
    'TOTAL', String(ct.camps), String(ct.destinations), num(ct.answered),
    num(ct.missed), num(ct.counted), '—', money2(ct.total_bill),
  ]
  campBody.push(campTotalRow)
  const campTotalIndex = campBody.length - 1
  ...
  autoTable(doc, {
    ...
    margin: { left: M, right: M },
    didParseCell: (data) => {
      if (data.section === 'body' && data.row.index === campTotalIndex) {
        data.cell.styles.fillColor = NAVY
        data.cell.styles.textColor = WHITE
        data.cell.styles.fontStyle = 'bold'
        if (data.column.index >= 2) data.cell.styles.halign = 'right'
