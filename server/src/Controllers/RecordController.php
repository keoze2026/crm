...
final class RecordController
{
    private const SELECT = "
        SELECT r.id, r.record_date, r.record_type,
               ...
               r.answered, r.missed, r.replacement, r.counted, r.rate, r.total_bill
        FROM call_records r
    ...
    public function store(): void
    {
        ...
        $stmt = $pdo->prepare(
            'INSERT INTO call_records
                (record_date, record_type, buyer_id, campaign_id, source, answered, missed, replacement, counted, rate)
             VALUES (:d, :t, :bid, :cid, :src, :a, :m, :rep, :c, :r) RETURNING id'
        );
        $stmt->execute([
            ':d' => $date, ':t' => $type, ':bid' => $buyerId, ':cid' => $campaignId, ':src' => $source,
            ':a'   => (int) ($body['answered'] ?? 0),
            ':m'   => (int) ($body['missed'] ?? 0),
            ':rep' => (int) ($body['replacement'] ?? 0),
    ...
    public function update(array $params): void
    {
        ...
        $stmt = $pdo->prepare(
            'UPDATE call_records SET
                ...
                replacement = COALESCE(:rep, replacement),
        ...
        $stmt->execute([
            ...
            ':rep' => isset($body['replacement']) ? (int) $body['replacement'] : null,
